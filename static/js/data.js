function getData() {
    ...
    let norms = [];
    for (let i = 0; i < 14; i++) {
        X0.push(parseFloat(document.getElementById(`X0_${i}`)?.value) || 0.5);
        norms.push(parseFloat(document.getElementById(`norm_${i}`)?.value) || 1.0);
    }

    // Полиномы 14×14×4 (заполняем нулями, потом перезаписываем значимые)
    let poly_coeffs = Array(14).fill(null).map(() => 
        Array(14).fill(null).map(() => [0, 0, 0, 0])
    );
    ...
    for (let i = 0; i < 14; i++) {
        for (let j = 0; j < 14; j++) {
            for (let c = 0; c < 4; c++) {
                ...
                if (input) {
                    poly_coeffs[i][j][c] = parseFloat(input.value) || 0;
                }
            }
        }
    }
    ...
    for (let k = 0; k < 5; k++) {
        ...
        for (let c = 0; c < 4; c++) {
            ...
            coeffs.push(input ? parseFloat(input.value) || 0 : 0);
        }
